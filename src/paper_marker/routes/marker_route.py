...
class MarkerRoute(ConversionRoute):
    name = "marker"

    def is_available(self) -> tuple[bool, str]:
        executable = resolve_cli_executable("marker")
        if executable:
            return True, f"Found marker CLI at {executable}"
        return False, "marker CLI not found on PATH or in the paper-marker environment"
    ...
    def convert(self, pdf_path: Path, work_dir: Path, timeout_s: int) -> CandidateResult:
        ...
        if not executable:
            return CandidateResult(
                ...
                status="unavailable",
                error="marker CLI not found on PATH or in the paper-marker environment",
                elapsed_s=time.perf_counter() - start,
            )
        cmd = [executable, str(pdf_path), "--output", str(out_dir)]
        try:
